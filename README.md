...
[DWM](https://dwm.suckless.org/) [customized](https://github.com/maicher/dwm) with following patches: noborder, pertag, fibonacci, centeredmaster, fakefullscreen.

## File Manager

[lf](https://github.com/gokcehan/lf) file manager, which is fast and also serves as an entry point for running scripts.
...
## How to manage the customized DWM fork

To keep the repo in sync with the upstream:
* commit all own modifications and patches to the `patch` branch,
* from time to time overwrite the origin with upstream and re-merge `patch` branch:

        git clone https://github.com/maicher/dwm
        cd dwm
        git checkout master
        git fetch upstream master
        git reset --hard upstream/master

At this point master is 1:1 upstream master.
The patches are on the patch branch, rebase it for clearer history:

    git checkout patch
    git rebase master

Then, re-merge patch branch:
